...
#
# $Id: libsent-config-dist.in,v 1.1 2008/10/02 15:01:59 sumomo Exp $
#

#
# Generated automatically from libsent-config-dist.in by configure.
#

sound_inc="-Isrc/adin/pa"
sound_libs=" -lwinmm -ldsound  -lz"
version="\
Julius/Julian libsent library rev.4.1.3"
info="\
****************************************************************
Julius/Julian libsent library rev.4.1.3:

- Audio I/O
    primary mic device API  : pa-dsound (PortAudio library (internal) (DirectSound))
...
while test $# -gt 0; do
    ...
    *) optarg= ;;
    ...
    case $1 in
	--cflags)
	    echo -I${prefix}/include $sound_inc
	    ;;
	--libs)
	    echo -L${exec_prefix}/lib -lsent $sound_libs  -lws2_32 -lm
	    ;;
	--version)
	    echo "${version}"
	    ;;
